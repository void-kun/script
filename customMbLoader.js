...
(function ($, undefined) {
  ...
  $.widget('nntoan.mbDownloader', {
    ...
    options: {
      ...
      insertMode: 'appendTo',
      credits:
        '<p>UserScript được viết bởi: <a href="https://nntoan.com/">Toan Nguyen</a></p>',
      general: {
        host: location.host,
      ...
      regularExp: {
        ...
        chineseSpecialChars:
          /[^\x09\x0A\x0D\x20-\uD7FF\uE000-\uFFFD\u10000-\u10FFFF]+/gm, //eslint-disable-line
      ...
      classNames: {
        ...
        ebookType: null,
        downloadBtnStatus:
          'btn-primary btn-success btn-info btn-warning btn-danger blue success warning info danger error',
    ...
    _create: function () {
      ...
      // Works with options
      this.options.general.referrer =
        this.options.general.referrer +
        this.options.general.host +
        this.options.general.pathname;
      this.options.xhr.content.url =
        this.options.general.pathname + this.options.chapters.chapId + '/';
      ...
      if (this.createDownloadWrapper === true) {
        this.elements.$downloadWrapper.attr(
          'class',
          this.options.classNames.downloadWrapper
        );
        this.elements.$downloadWrapper.html(this.elements.$downloadBtn);
        this.elements.$downloadWrapper.append(
          this.options.classNames.downloadAppendTo
        );
      } else {
    ...
    getBookInfo: function () {
      ...
      options.ebook = $.extend(options.ebook, {
        ...
        author: $infoBlock
          .find(options.classNames.ebookAuthor)
          .find('p')
          .text()
          .trim(),
        cover: $infoBlock
          .find(options.classNames.ebookCover)
          .find('img')
          .attr('src'),
      ...
      if (epubInfo.hasOwnProperty('fallbackCover'))
        delete epubInfo.fallbackCover;
    ...
    updateChapId: function (that) {
      ...
      options.chapters.chapId =
        options.chapters.chapList[that.processing.count];
      options.xhr.content.url =
        options.general.pathname + options.chapters.chapId + '/';
    ...
    processListOfChapters: function (response, that, $widget) {
      ...
      options.chapters.chapList = options.chapters.chapList
        .map(function (val) {
      ...
      if (event.type === 'contextmenu') {
        $widget.off('click');
        var startFrom = prompt(
          'Nhập ID chương truyện bắt đầu tải:',
          options.chapters.chapList[0]
        );
        startFrom = options.chapters.chapList.indexOf(startFrom);
        if (startFrom !== -1) {
          options.chapters.chapList =
            options.chapters.chapList.slice(startFrom);
        }
      } else {
        ...
        $.ajax(options.xhr.content)
          .done(function (response) {
            ...
            $widget.html(
              'Đang tải: ' +
                Math.floor(
                  (self.processing.count / self.processing.chapListSize) * 100
                ) +
                '%'
            );
            ...
            document.title =
              '[' + self.processing.count + '] ' + options.general.pageName;
    ...
    updateChapterTitle: function (that, $result) {
      ...
      options.chapters.chapTitle = $result
        .find(options.classNames.chapterTitle)
        .text()
        .trim();
        ...
        if ($chapter.find(options.classNames.chapterVip).length) {
          ...
          $chapter.filter(function () {
            return (
              this.textContent
                .toLowerCase()
                .indexOf('vui lòng đăng nhập để đọc chương này') !== -1
            );
          }).length
        ) {
          ...
          if ($img.length) {
            $img.replaceWith(function () {
              return (
                '<br /><a href="' + this.src + '">Click để xem ảnh</a><br />'
              );
            });
          }
    ...
    chapListValueFilter: function (options, val) {
      if (typeof options.chapters.chapListSlice[1] !== 'undefined') {
        val = val.slice(
          options.chapters.chapListSlice[0],
          options.chapters.chapListSlice[1]
        );
      } else {
    ...
    downloadStatus: function (status) {
      ...
      self.elements.$downloadBtn
        .removeClass(options.classNames.downloadBtnStatus)
        .addClass('btn-' + status)
        .addClass(status);
    },
    ...
    downloadError: function (error, message, $widget, retry) {
      ...
      return (
        '<p class="no-indent"><a href="' +
        options.general.referrer +
        options.chapters.chapId +
        '">' +
        message +
        '</a></p>'
    ...
    saveEbook: function ($widget) {
      ...
      if (self.processing.titleError.length) {
        self.processing.titleError =
          '<p class="no-indent"><strong>Các chương lỗi: </strong>' +
          self.processing.titleError.join(', ') +
          '</p>';
      } else {
        self.processing.titleError = '';
      }
      self.processing.beginEnd =
    ...
    generateEpub: function (that, $widget) {
      var options = that.options,
        ebookFilepath =
          options.processing.ebookFileName + options.processing.ebookFileExt;
